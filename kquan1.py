...
# =====================================================================================
# PHẦN 1: MÃ NGUỒN TỪ KQUAN1.PY (LOGIC CHÍNH CỦA TOOL)
# =====================================================================================
...
def top_10_cdtd(headers):
    params = ''
    try:
        response = requests.get('https://api.sprintrun.win/sprint/recent_10_issues', params=params,
    ...
    for champion_id in selected_champions:
        try:
            ...
            if response.get('code') == 0 and response.get('msg') == 'ok':
                prints(0, 255, 19,
                       f"    -> Đã đặt {config['current_coins']:.4f} {config['Coin']} cho '{NV[champion_id]}' thành công.")
            else:
                prints(255, 0, 0,
                       f"    -> Lỗi khi đặt cho '{NV[champion_id]}': {response.get('msg', 'Lỗi không xác định')}")
        except Exception as e:
...
# =====================================================================================
# PHẦN 2: MÃ NGUỒN TỪ KEY.PY (LOGIC XÁC THỰC)
# =====================================================================================
...
def get_device_id():
    ...
    try:
        if system == "Windows":
            cpu_info = subprocess.check_output('wmic cpu get ProcessorId', shell=True, text=True,
...
def generate_key_and_url(ip_address):
    ...
    url = f'https://tdkkeyfree.blogspot.com/2025/09/t1.html?m={key}'  # Link này có thể giữ nguyên hoặc thay đổi tùy admin
    ...
    if cached_vip_info and cached_vip_info.get('device_id') == device_id:
        try:
            ...
            if expiry_date.date() >= datetime.now().date():
                ...
                key_info = {'type': 'VIP', 'key': cached_vip_info['key'],
                            'expiry': cached_vip_info['expiration_date']}
                time.sleep(3)
...
# =====================================================================================
# PHẦN 3: ĐIỂM BẮT ĐẦU CHƯƠNG TRÌNH
# =====================================================================================
if __name__ == "__main__":
    authentication_successful, _, _ = main_authentication()

    if authentication_successful:
        try:
            # Nếu xác thực thành công, chạy logic chính của tool
            main_cdtd()
        except KeyboardInterrupt:
        ...
        print(f"{do}Xác thực không thành công. Thoát chương trình.{trang}")
        time.sleep(3)
        sys.exit()
